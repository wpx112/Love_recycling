...
from mysetting import phone_path,pid,n
import re
import json

phone_path = r'D:\wpx\workspace\Love_recycling\test\test_ids\tt.txt'
pid = 27512
n = 4
def get_base_infos(url,num):  # 获取所有靓机价的基本信息id组合
    ...
    if r.status_code == 200:
        ...
        # print('--------------该处可以判断n-----------------')
        # print(base_infos_str)
        # l = json.loads(base_infos_str)
        # print(len(l[0])) # 该处可以判断n
        # print(type(base_infos_str))
        # print("---------------该处可以判断n------------------")
        ...
        if base_infos_str == '[[]]':
            ...
            # print('--------------该处可以判断base_infos_lst -----------------')
            # print(base_infos_lst)
            # print("---------------该处可以判断base_infos_lst ------------------")
            n = num

            base_infos = [base_infos_lst[i:i + n] for i in range(0, len(base_infos_lst), n)]
            print('--------------该处可以判断base_infost -----------------')
            print(base_infos)
            print("---------------该处可以判断base_infos ------------------")
...
def generate_id_options(data_ids, qx_id='4475'):  # 生成所有估价的id选项组合(不包含基本信息), qx_id为全新机选项id
    id_options_all = []
...
def generate_id_units(id_options, base_infos, qx_id='4475'):  # 生成所有包含基本信息id选项组合
